...
import { MyConsumptionView } from './MyConsumptionView';
import { ProfileView as TeacherProfileView } from './ProfileView';
...
// FIX: Replaced the useInterval implementation with a more robust version that avoids potential race conditions and cleans up the logic.
...
const DashboardHome: React.FC<{ students: Student[] }> = ({ students }) => (
    <Card>
        <CardHeader><h2 className="text-xl font-semibold">Resumen del Día</h2></CardHeader>
        <CardContent>
            <p>Tienes {students.length} estudiantes en tus cursos.</p>
        </CardContent>
    </Card>
);
...
const PickupView: React.FC<{ students: Student[] }> = ({ students }) => {
    ...
    const handleExportCSV = () => {
        ...
        const rows = reportData.map(record => {
            // FIX: Replace studentMap lookup with a more reliable .find() on the students array to fix type error.
    ...
    return (
        <div>
            ...
            <div className="flex border-b mb-4">
                <button onClick={() => setActiveTab('today')} className={`px-4 py-2 text-sm font-medium ${activeTab === 'today' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>Recogidas de Hoy</button>
                <button onClick={() => setActiveTab('reports')} className={`px-4 py-2 text-sm font-medium ${activeTab === 'reports' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>Historial y Reportes</button>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <Card>
                        <CardHeader className="flex justify-between items-center">
                            <h2 className="text-xl font-semibold">Recogida en Vivo</h2>
                            ...
                            {livePickups.length > 0 ? livePickups.map(p => (
                                <div key={p.studentId} className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                    <div className="flex justify-between items-center">
                                        <div>
                                            {/* FIX: Use the studentName property directly from the live pickup data to fix type error. */}
                                    ...
                                    <Button className="w-full mt-3" onClick={() => handleRecordPickup(PickupType.Notified, p.studentId, p.parentName)}>Entregado</Button>
                                </div>
                            )) : <p className="text-center text-gray-500 py-8">Nadie ha notificado su llegada aún.</p>}
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader><h2 className="text-xl font-semibold">Recogida Programada</h2></CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <div>
                                <Label htmlFor="parentName">Nombre del Padre/Madre/Autorizado</Label>
                            ...
                            {selectedParent === 'other' && (
                                <div>
                                    <Label htmlFor="otherParentName">Especifique el nombre</Label>
                            ...
                            <Button className="w-full" onClick={() => handleRecordPickup(PickupType.Scheduled, selectedStudentId, selectedParent === 'other' ? otherParentName : selectedParent)}>Registrar Salida</Button>
                        </CardContent>
                    </Card>
                </div>
            )}
            ...
            {activeTab === 'reports' && (
                <Card>
                    <CardHeader><h2 className="text-xl font-semibold">Generar Reporte de Recogidas</h2></CardHeader>
                    <CardContent>
                        <div className="flex flex-col md:flex-row items-center gap-4 mb-4 pb-4 border-b">
                            <div><Label>Desde:</Label><Input type="date" value={reportStartDate} onChange={e => setReportStartDate(e.target.value)} /></div>
                            ...
                            <Button onClick={handleExportCSV} disabled={reportData.length === 0} variant="secondary" className="self-end">Exportar a Excel</Button>
                        </div>
                        <div className="overflow-x-auto max-h-96">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50 sticky top-0"><tr className="text-left text-xs font-medium text-gray-500 uppercase">
                                    <th className="px-4 py-2">Fecha</th><th className="px-4 py-2">Hora</th><th className="px-4 py-2">Estudiante</th><th className="px-4 py-2">Recogido por</th><th className="px-4 py-2">Tipo</th><th className="px-4 py-2">Tardanza (min)</th>
                                </tr></thead>
                            ...
                            {reportData.length === 0 && <p className="text-center text-gray-500 p-4">No hay datos para el rango de fechas seleccionado.</p>}
                        </div>
                    </CardContent>
                </Card>
            )}
...
const MyCoursesView: React.FC<{ students: Student[] }> = ({ students }) => {
    ...
    const refreshStudentData = () => {
        // Here you would refetch the student data in a real app
...
const CommunicationsView: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            // Pre-build maps for quick lookups
    ...
    const handleSendMessage = async () => {
        ...
        // If no conversation exists, we can't send a message. 
        // A real app might create one, but our mock API doesn't support that flow easily.
        if (!convToUse) {
    ...
    return (
        <div className="flex h-[calc(100vh-200px)] bg-white rounded-xl shadow-md overflow-hidden">
            {/* Student List */}
            <div className="w-1/3 border-r flex flex-col overflow-hidden">
                <div className="p-4 border-b">
                    <Input placeholder="Buscar estudiante..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                </div>
                <ul className="overflow-y-auto flex-1">
                    {filteredStudents.map(student => {
                        ...
                        const conv = parentId ? parentToConversationMap.current.get(parentId) : null;
                        return (
                            <li key={student.id} onClick={() => handleSelectStudent(student)} className={`flex items-center p-3 cursor-pointer hover:bg-gray-100 ${selectedStudent?.id === student.id ? 'bg-blue-50' : ''}`}>
                                <img src={student.avatarUrl} alt={student.name} className="w-12 h-12 rounded-full" />
                                <div className="ml-3 flex-grow">
                                    <p className="font-semibold">{student.name}</p>
                                    <p className="text-sm text-gray-500 truncate">{conv?.lastMessage.text || 'Sin mensajes'}</p>
                                </div>
                ...
                {selectedStudent ? (
                    <>
                        <div className="p-4 border-b flex items-center">
                            ...
                            <h3 className="text-lg font-semibold ml-3">Conversación con padre/madre de {selectedStudent.name}</h3>
                        </div>
                        <div className="flex-grow p-4 overflow-y-auto bg-gray-50">
                            {isLoadingMessages ? <p>Cargando...</p> : messages.map(msg => {
                                const sender = users.get(msg.senderId);
                                return (
                                    <div key={msg.id} className={`flex items-end gap-2 ${msg.senderId === currentUser?.id ? 'justify-end' : 'justify-start'} mb-4`}>
                                        {msg.senderId !== currentUser?.id && <img src={sender?.avatarUrl} className="w-6 h-6 rounded-full" />}
                                        <div className={`max-w-md p-3 rounded-lg ${msg.senderId === currentUser?.id ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-200 text-gray-800 rounded-bl-none'}`}>
                                            <p>{msg.content}</p>
                                        </div>
                                    </div>
                                )
                        ...
                        <div className="p-4 border-t bg-white">
                            ...
                            <div className="flex justify-between items-center mt-2">
                                <div className="space-x-2">
                                    <Button size="sm" variant="secondary" onClick={() => handleQuickAction('[TAREA]')}>Tarea</Button>
                                    <Button size="sm" variant="secondary" onClick={() => handleQuickAction('[NOTA DE COMPORTAMIENTO]')}>Nota</Button>
                                    <Button size="sm" variant="secondary" onClick={() => handleQuickAction('[SOLICITUD ESPECIAL]')}>Solicitud</Button>
                                </div>
                                <Button onClick={handleSendMessage} disabled={!currentConversation}>Enviar</Button>
                            </div>
                        </div>
                    </>
                ) : (
            ...
            api.getStudentsByTeacher(currentUser.id).then(setStudents);
             if (setNotificationCounts) {
                api.getConversationsForUser(currentUser.id).then(convos => {
                    ...
                    setNotificationCounts((prev: any) => ({ ...prev, 'Comunicados': unreadCount }));
                });
            }
        }
