...
interface StudentEnrollmentFormViewProps {
  studentId: string;
  onBack: () => void;
}
...
export const StudentEnrollmentFormView: React.FC<StudentEnrollmentFormViewProps> = ({ studentId, onBack }) => {
    ...
    const handleShare = () => {
        const url = `${window.location.origin}/enroll/${studentId}/${Date.now()}`;
        alert(`ENLACE SIMULADO PARA COMPARTIR CON PADRES:\n\n${url}\n\nEste enlace permitiría a los padres llenar este formulario.`);
    };
    
    const handleSave = async () => {
        if (!student) return;
        setIsSaving(true);
        // Mark profile as complete when admin saves it
    ...
    if (!student) return <div>Estudiante no encontrado. <Button onClick={onBack}>Volver a la lista</Button></div>;
    ...
    const renderStudentData = () => (
      <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div><Label>Nivel a cursar</Label><Input value={student.gradeLevel} onChange={e => handleNestedChange('gradeLevel', e.target.value)} /></div>
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <Button variant="secondary" onClick={onBack}>&larr; Volver a la Lista</Button>
                <div className="flex space-x-2">
                    <Button variant="secondary" onClick={handleShare}>Compartir Formulario</Button>
                    <Button onClick={handleSave} disabled={isSaving}>{isSaving ? 'Guardando...' : 'Guardar Perfil'}</Button>
                </div>
            </div>
            ...
            <Card>
                <CardHeader>
                    <div className="flex border-b border-gray-200">
                        <button onClick={() => setActiveTab('student')} className={`px-4 py-2 text-sm font-medium ${activeTab === 'student' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>Datos del Estudiante</button>
